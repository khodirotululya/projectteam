...
class M_Tap extends CI_Model{
    ...
    function get_Laporan($id){
        ...
        $r = $this->db->query("SELECT tanggal FROM laporankas where id = '$id'")->row_array();
        $t = $r['tanggal'];
    ...
    function update_kasbos($tipe,$nominal){
        $ini = $this->db->query("UPDATE laporanbos SET $tipe = $tipe + '$nominal'  WHERE tanggal = DATE(NOW())");
        return;             
    }

    function get_LaporanBos($id){
        $r = $this->db->query("SELECT tanggal FROM laporanbos where id= '$id'")->row_array();
        $t = $r['tanggal'];

        $danabos = $this->db->query("SELECT nominal, keterangan FROM danabos WHERE time = '$t'")->result();
        
        return array('danabos'=>$danabos,'tanggal'=>$t);
